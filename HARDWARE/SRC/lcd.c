...
void LCD_ShowFloat(u16 x, u16 y, double num, u8 len1, u8 len2, u8 size)
{
	u32 integ = (u32)num;
	u32 rest = (u32)((num-(double)integ)*LCD_Pow(10, len2));
	LCD_ShowNum(x, y, integ, len1, size);
	LCD_ShowNum(x+size*(len1+1)/2, y, rest, len2, size);
	LCD_ShowChar(x+size*len1/2, y, (u8)'.', size, 0);
	return;
}
